...
| `superadmin` | All admin abilities **plus** owns businesses + quests directly, bypasses the verification queue (auto-approved on create, edits stay approved), and is exempt from the paid-promotion gate on the Recommendation Status card |
...
| 2026-05-19 | Player progress model: `travel_challenge_progress` + per-stop `player_status` on `challenge_completions`; stack Roll picks random incomplete stop, Route costs 1 skip; 3 skip tokens / 3h refill | Derive everything client-side; keep dice-as-movement for travel sets | Roll/Route semantics must survive stacks larger than six later — pool size is dynamic, publish minimum stays six today. Server owns lifecycle (`available` → `ongoing` → `submitted` → `claimed` / `expired`) so iOS, web, and future Android share one contract | All |
| 2026-05-22 | Per-quest skip budget on `travel_challenge_progress.skips_used/skips_limit` instead of a single global pool for travel quests | Reuse the global `profiles.free_skips_used` counter; per-stop counter on completions | The product flow wants "3 skips per quest, then non-blocking side ads." A global pool drained across quests, and a per-stop counter doesn't help with skips that re-roll without ever picking. Per-quest skips also reset cleanly whenever the user starts a fresh `travel_challenge_progress` row. The legacy global pool still runs for nearby Roll/Route browse so the existing UX is unchanged outside quests | All |
| 2026-05-22 | Big-reward redemption via `travel_challenge_progress.big_reward_claim_code` + `TT-BR-*` codes flowing through `/v1/redemptions/{lookup,verify}` | New `/v1/redemptions/big-reward-*` routes; encode claim into the QR alone without server persistence | Reusing the existing merchant scanner endpoints means no new client surface for merchants — the same QR-scan flow handles per-stop verifications and quest wins. Persisting the claim code lets the merchant look it up before approving and prevents code re-use after redemption | All |
| 2026-05-27 | Rename user-facing "Travel Challenge" → "Quest" (cosmetic only; tables, API paths, types keep `travel_challenge*`) | Full rename including code identifiers / migrations (`travel_challenges` → `quests`, `/v1/travel-challenges/*` → `/v1/quests/*`, `TravelChallenge` Swift types → `Quest`) | Risk of breaking the iOS↔web wire contract on a live MVP for a copy change; cosmetic rename gives users a single consistent term (Home, Map, Quest Preview, share sheet, admin nav) with zero migration risk. Doc note in `QUEST_FLOW.md` flags that backtick-formatted `travel_challenge*` identifiers stay as-is | iOS / Web / Future Android |
| 2026-06-04 | Merchant business-count plans (`free`=1, `growth`=3, `unlimited`) stored on `profiles.plan_code`, enforced by a `BEFORE INSERT` trigger on `businesses` (migration 035), provider-agnostic | Enforce only in the web Server Action; overload the promotion `merchant_subscriptions.tier`; per-merchant `business_limit` column | The cap is a data-integrity rule, so it lives in Postgres and holds for every client (iOS/web/Android), not just the dashboard. Keeping the entitlement on `profiles` (not the promotion-subscription table) avoids conflating "promote a challenge" with "how many businesses you may own." `plan_business_limit()` is the single allowance source; admin/superadmin and service-role inserts bypass | All |
| 2026-06-04 | Stripe for merchant plan billing (subscription Checkout + Billing Portal + webhook → `profiles.plan_code`), env-guarded with a mock-safe fallback | Xendit recurring (already integrated, PH-native), PayMongo | Chosen pending confirmation that the collecting entity is in a Stripe-supported country — **Stripe cannot onboard PH-registered businesses**. Billing kept on the web dashboard only (not the iOS app) to avoid Apple's IAP 30% requirement on digital subscriptions. The payment provider only flips the plan flag, so swapping to a PH gateway later reuses the entire plan model and trigger | Web (billing), All (entitlement) |
| 2026-06-15 | Promotion tier switch supersedes the prior promotion: `startSubscription` + the Xendit webhook cancel the merchant's other `active` `merchant_subscriptions` once the new one is active; `expire_merchant_subscriptions()` (migration 045) flips past-`ends_at` actives to `expired` hourly via pg_cron | Allow multiple concurrent promotions; only filter by `ends_at` at read time and leave stale `status='active'` rows; enforce single-active with a partial unique index | A merchant runs one promotion at a time, so a tier switch must end the previous one — previously every switch inserted a fresh `active` row and stacked them. Cancelling on activation (not on insert) keeps live-mode coverage gap-free until the new charge clears. The hourly expiry keeps stored `status` consistent with the `ends_at` window so the Promote History badge and recommendation reads agree without depending on each query re-deriving expiry | Web (billing), All (recommendation visibility) |
| 2026-06-18 | Quest expiry is fully retroactive: `expire_quests()` (migration 046, replacing 038) flips quests past `date_range_end` to `expired`, expires every still-`active` player session on a dead/past-window quest, AND expires the in-flight `ongoing` per-stop `challenge_completions` on those sessions | Only block new joins via the accept route; expire the quest but leave joined players `active`; client-side date check only | "End date exceeded" must stop *everyone already playing*, not just new entrants — leaving sessions/stops active let joined travelers keep verifying a dead quest. Expiring all three layers in one idempotent function (run hourly + once retroactively) means read paths (`loadActiveTravelProgress` filters `active`, `derivePlayerStopStatus` maps `expired`) drop the quest from Home "Continue" and the map immediately, with no client logic needed | All |
| 2026-07-04 | Approved businesses are mirrored into `places` (migration 047: `places.business_id` + `sync_business_place()` trigger on insert/approval/coord changes, `prewarmed` so the 24h cache TTL never evicts them; merged by `google_place_id` when the business is pinned to a Google POI) | Inject business rows into `/v1/places` responses at request time; require merchants to wait until Google's nearby search returns their venue; a separate "merchant places" feed in the app | Home Trending/Nearby cards come exclusively from Google-mirrored `places`, so a newly approved merchant (Palo Verde Resort) was invisible unless Google happened to return their venue. Mirroring at approval time makes the business a first-class POI — distance-ranked in Nearby, red-dot annotated, and `/v1/places/:id` detail works — with no request-time special-casing. Promotion still gates the dice-roll pool, not POI presence | All |
| 2026-07-04 | `recommended_challenges` (migration 048) excludes stops whose parent quest is not `live`/`approved` (standalone challenges unaffected) | Leave the pool keyed on child status only; expire child stops alongside the quest in `expire_quests()` | Quest expiry (046) leaves child stops `live` (they're reusable if the quest is renewed), so expired-quest stops kept surfacing in the dice roll while the accept route rejected them — a roll that can never be accepted. Gating the view on the same quest statuses the accept route allows keeps roll and accept consistent, and a renewed quest resurfaces its stops with no data churn | All |
| 2026-07-04 | Traveler "Unlimited" subscription moves to StoreKit 2 IAP (yearly $4.99 / monthly $1.99): in-app paywall, on-device verified entitlement mirrored to `profiles.is_unlimited` via `POST /v1/me/entitlement`, and skip functions honor the pass server-side (migration 049: `consume_quest_skip` / `consume_skip_token` return consumed without decrementing for unlimited users). Stripe checkout removed from the iOS app (webhook path kept for legacy/web) | Keep Stripe checkout in-app (previous decision); StoreKit + App Store Server Notifications from day one | Selling a consumer digital subscription in-app via external checkout violates App Store guideline 3.1.1 — a guaranteed rejection. StoreKit 2 gives verified transactions, restore, and local `.storekit` testing with no server dependency; mirroring the entitlement to `profiles` lets existing server gates (unlimited skips, future no-ads flags) work unchanged. Server Notifications can harden revocation later without changing the contract | iOS (paywall/ads), All (entitlement) |
| 2026-07-04 | ONE skip economy: quest skip/abandon routes consume the GLOBAL pool (`consume_skip_token`: 3 skips, 3h refill via `refill_free_skips_if_due`, unlimited for subscribers) instead of the per-quest `travel_challenge_progress.skips_used` budget (reverses 2026-05-22); responses and the quest-detail progress payload translate the pool into the legacy `{skips_used, skips_limit}` wire shape so shipped builds keep decoding | Keep two parallel pools (per-quest + global); migrate the wire shape too | Two parallel skip systems double-charged one perceived action (re-roll spent the quest budget, dismiss-then-roll spent it again → "3 left" jumped to 1) and the per-quest budget never recharged, contradicting the product spec ("3 skips per user, refills every 3 hours, shared across all challenges; subscribers unlimited"). One pool makes every surface agree, keeps the 3h recharge, and needs no schema change — the per-quest columns simply stop being charged | All |
